...
export function AccountProfilePanel() {
  ...
  const avatarUrl = data?.user.avatarUrl || user?.avatarUrl;
      ...
      <Card className="bg-white/5 border-white/10">
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <User className="h-4 w-4 text-amber-400" /> Profile & Identity
          </CardTitle>
          <CardDescription>Your personal account information used across deposits, withdrawals, and KYC.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="flex flex-col sm:flex-row items-start gap-4">
            <div className="relative">
              <Avatar className="h-20 w-20 border-2 border-amber-500/30">
                {avatarUrl ? <AvatarImage src={avatarUrl} alt={form.fullName} /> : null}
                <AvatarFallback className="bg-amber-500/10 text-amber-400 text-2xl font-bold">{initials}</AvatarFallback>
              </Avatar>
            ...
            <div className="flex-1 space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                <Badge className="capitalize bg-white/10 border-white/10">{data?.user.role}</Badge>
                <Badge className="capitalize bg-amber-500/10 text-amber-400 border-amber-500/20">{data?.user.kycStatus}</Badge>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))}
                className="bg-white/5 border-white/10" placeholder="+91..." />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs text-muted-foreground">Username</Label>
              <Input value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value.toLowerCase() }))}
                className="bg-white/5 border-white/10" placeholder="optional" />
            </div>
            <div className="space-y-1.5">
