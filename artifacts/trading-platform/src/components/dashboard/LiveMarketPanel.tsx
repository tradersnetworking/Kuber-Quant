...
function MarketTickCard({ tick }: { tick: MarketTick }) {
  ...
  return (
    <div className="shrink-0 w-[140px] sm:w-[152px] p-3 rounded-lg bg-white/[0.03] border border-white/5 hover:border-white/10 transition-colors snap-start">
      <p className="text-xs font-semibold text-muted-foreground mb-1 truncate">{tick.symbol}</p>
      <p className="text-lg font-black font-mono text-white">{formatMarketPrice(tick.symbol, tick.price)}</p>
      {tick.changePercent !== 0 ? (
        <div className={cn("flex items-center gap-1 mt-1 text-xs font-medium", up ? "text-green-400" : "text-red-400")}>
          {up ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
          {up ? "+" : ""}{tick.changePercent.toFixed(2)}%
        </div>
      ) : tick.price > 0 ? (
        <p className="text-[10px] text-zinc-500 mt-1">Live</p>
      ) : (
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <CardTitle className="text-base font-bold flex items-center gap-2">
              <Radio className={cn("h-4 w-4", ticks.length ? "text-green-400 animate-pulse" : "text-zinc-500")} />
              Live Markets
              <Badge className="bg-green-500/10 text-green-400 border-green-500/20 text-xs">{watchlist.length}/10 pairs</Badge>
              {dataSource === "direct" && (
                <Badge variant="outline" className="text-[10px] border-amber-500/30 text-amber-400">Direct feed</Badge>
              )}
            </CardTitle>
          ...
          <div className="mb-4 p-4 rounded-lg border border-amber-500/20 bg-amber-500/5 space-y-3">
            ...
            <div className="flex gap-2 flex-wrap">
              <Select value={addPair} onValueChange={setAddPair}>
                <SelectTrigger className="w-[160px] h-8 bg-white/5 border-white/10 text-xs"><SelectValue placeholder="Add pair..." /></SelectTrigger>
                <SelectContent>
          ...
          <div className="relative group">
            {canScrollLeft && (
              <button
                ...
                className="absolute left-0 top-1/2 -translate-y-1/2 z-10 h-8 w-8 rounded-full bg-black/80 border border-white/10 text-amber-400 hover:border-amber-500/40 flex items-center justify-center shadow-lg md:opacity-0 md:group-hover:opacity-100 transition-opacity"
            ...
            {canScrollRight && (
              <button
                type="button"
                onClick={() => scrollBy(320)}
                className="absolute right-0 top-1/2 -translate-y-1/2 z-10 h-8 w-8 rounded-full bg-black/80 border border-white/10 text-amber-400 hover:border-amber-500/40 flex items-center justify-center shadow-lg md:opacity-0 md:group-hover:opacity-100 transition-opacity"
