...
const RISK_CONFIG: Record<string, { color: string; bg: string; bar: number }> = {
  "Very Low":  { color: "text-blue-400",   bg: "bg-blue-500/20",   bar: 10 },
  "Low":       { color: "text-green-400",  bg: "bg-green-500/20",  bar: 25 },
  "Medium":    { color: "text-amber-400",  bg: "bg-amber-500/20",  bar: 50 },
  "High":      { color: "text-orange-400", bg: "bg-orange-500/20", bar: 75 },
  ...
  return (
    <AppLayout>
      <div className="space-y-6">
        ...
        {/* Hero */}
        <div className="bg-gradient-to-r from-black/60 to-amber-900/10 border border-white/10 rounded-2xl p-6 md:p-8">
          <div className="flex flex-col md:flex-row md:items-start gap-6">
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-2 mb-2">
                <Badge className="bg-amber-500/20 text-amber-400 text-xs">{strategy.category}</Badge>
                <Badge className={`text-xs ${risk?.bg} ${risk?.color}`}>{strategy.riskLevel} Risk</Badge>
                <Badge className="bg-blue-500/20 text-blue-400 text-xs">{strategy.platform.toUpperCase()}</Badge>
                {mySubscription && <Badge className="bg-green-500/20 text-green-400 text-xs flex items-center gap-1"><CheckCircle className="h-2.5 w-2.5" />Active Subscription</Badge>}
              </div>
              <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">{strategy.name}</h1>
              <p className="text-muted-foreground leading-relaxed mb-4">{strategy.description}</p>
              <div className="flex flex-wrap gap-2">
                {tags.map(t => (
                  <span key={t} className="text-xs bg-white/5 border border-white/10 rounded-full px-3 py-1 text-muted-foreground">{t}</span>
                ))}
              </div>
            </div>
            <div className="flex flex-col gap-2 md:min-w-[160px]">
              {mySubscription ? (
                <Button onClick={handleDownload} disabled={downloading}
                  ...
                  {downloading ? "Downloading..." : "Download EA"}
                </Button>
              ) : (
                <Button onClick={() => setSubDialog(true)}
                  className="bg-gradient-to-r from-amber-400 to-yellow-600 text-black font-bold">
                  <Lock className="h-4 w-4 mr-2" />Subscribe Now
                </Button>
              )}
              <p className="text-center text-xs text-muted-foreground">
                From <span className="text-amber-400 font-bold">${strategy.priceMonthly}/mo</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { icon: TrendingUp, label: "Backtest ROI", value: `${strategy.backtestRoi}%`, color: "text-green-400" },
                { icon: Target, label: "Win Rate", value: `${strategy.winRate}%`, color: "text-blue-400" },
                { icon: BarChart2, label: "Risk Level", value: strategy.riskLevel, color: risk?.color || "text-amber-400" },
                { icon: Users, label: "Subscribers", value: `${Math.floor(Math.random() * 800 + 50)}+`, color: "text-purple-400" },
              ].map(s => (
            ...
            <Card className="bg-white/5 border-white/10">
              <CardHeader><CardTitle className="text-base flex items-center gap-2"><Activity className="h-4 w-4 text-amber-400" />Strategy Details</CardTitle></CardHeader>
              <CardContent>
            ...
            {/* Risk Meter */}
            <Card className="bg-white/5 border-white/10">
              <CardHeader><CardTitle className="text-base flex items-center gap-2"><Shield className="h-4 w-4 text-amber-400" />Risk Assessment</CardTitle></CardHeader>
            ...
            <Card className="bg-white/5 border-white/10">
              <CardHeader><CardTitle className="text-base flex items-center gap-2"><BarChart2 className="h-4 w-4 text-amber-400" />Performance Highlights</CardTitle></CardHeader>
              <CardContent>
                <div className="grid grid-cols-3 gap-4 mb-4">
                  {[
                    ...
                    <div key={s.label} className="text-center bg-black/30 rounded-lg p-3 border border-white/5">
            ...
            <Card className="bg-white/5 border-white/10">
              ...
              <CardContent className="space-y-3">
                {[
                  ...
                  <details key={item.q} className="group bg-black/20 border border-white/5 rounded-lg">
          ...
          <div className="space-y-4">
            ...
            <Card className="bg-white/5 border-white/10 sticky top-4">
              ...
              <CardContent className="space-y-3">
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          <p className={`text-sm font-medium ${isSelected ? "text-amber-400" : "text-white"}`}>{plan.label}</p>
                          <p className="text-xs text-muted-foreground">{plan.days} days access</p>
                        </div>
                        <div className="text-right">
                          <p className={`text-lg font-bold ${isSelected ? "text-amber-400" : "text-white"}`}>${price}</p>
                          {key !== "monthly" && <p className="text-xs text-green-400">${(price / (plan.days / 30)).toFixed(0)}/mo</p>}
                        </div>
                      </div>
                    </button>
                  );
                ...
                {mySubscription ? (
                  <div className="space-y-2">
                    <Button onClick={handleDownload} disabled={downloading} className="w-full bg-gradient-to-r from-amber-400 to-yellow-600 text-black font-bold">
                      <Download className="h-4 w-4 mr-2" />{downloading ? "Downloading..." : "Download EA (.ex5)"}
                    </Button>
                  ...
                  <Button onClick={() => setSubDialog(true)} className="w-full bg-gradient-to-r from-amber-400 to-yellow-600 text-black font-bold h-12">
                    <Lock className="h-4 w-4 mr-2" />Subscribe — ${strategy[PLAN_PRICES[subForm.plan]?.key] ?? strategy.priceMonthly}
                  </Button>
                )}
            ...
            <Card className="bg-white/5 border-white/10">
              <CardContent className="p-4 space-y-2">
                {[
                  ...
                  <div key={item.label} className="flex justify-between text-xs py-1.5 border-b border-white/5 last:border-0">
        ...
        <Dialog open={subDialog} onOpenChange={setSubDialog}>
          <DialogContent className="bg-[#0a1628] border-white/10 max-w-md">
            <DialogHeader>
            ...
            <form onSubmit={handleSubscribe} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {["mt4", "mt5"].map(p => (
                    <button key={p} type="button"
                      ...
                      className={`py-2 rounded-lg border text-sm font-medium transition-all flex items-center justify-center gap-2 ${
              ...
              <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3">
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span>{PLAN_PRICES[subForm.plan]?.label}</span>
                </div>
                <div className="flex justify-between text-sm font-bold">
                  <span>Total</span>
                  <span className="text-amber-400">${strategy[PLAN_PRICES[subForm.plan]?.key] ?? strategy.priceMonthly}</span>
                </div>
              </div>
              <Button type="submit" disabled={subscribing || !subForm.mtAccountNumber}
                className="w-full bg-gradient-to-r from-amber-400 to-yellow-600 text-black font-bold h-12">
                {subscribing ? "Activating..." : "Confirm Subscription"}
              </Button>
            </form>
          </DialogContent>
        </Dialog>
