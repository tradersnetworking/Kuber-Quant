...
router.get("/subscriptions/:id/download", requireAuth, async (req, res) => {
  ...
  if (new Date() > sub.expiresAt) {
    res.status(403).json({ error: "Subscription expired. Please renew at kubercapital.com" }); return;
  }
...
//| Generated by Kuber Quant — kubercapital.com
...
//| Plan: ${sub.plan}
//+------------------------------------------------------------------+
#property copyright "Kuber Quant — kubercapital.com"
#property link      "https://kubercapital.com"
#property version   "1.00"
#property strict

input string  LicenseKey       = "${sub.licenseKey}";
input string  LicensedAccount  = "${sub.mtAccountNumber}";
input string  SupportURL       = "https://kubercapital.com/ea-strategies";
